...
const EnglishQuiz = () => {
  ...
  const nextQuestion = () => {
    if (lock === true) {
      ...
      if (index === EngQuizData.length - 1) {
        setResult(true);
        return 0; //so it not execute remaining statement
      }
      setIndex(++index);
      setQuestion(EngQuizData[index]);
  ...
  const [items, setItems] = useState(() => {
    const savedItems = localStorage.getItem("NNEng");
    return savedItems ? JSON.parse(savedItems) : [];
  });

  useEffect(() => {
    localStorage.setItem("NNEng", JSON.stringify(items));
  }, [items]);
    ...
    <>
      ...
      <div className="container">
        ...
        {result ? (
          <>
            <h2>
              You scored {score} our of {EngQuizData.length}
            </h2>
          ...
          <>
            ...
            <div className="index">
              {index + 1} of {EngQuizData.length} questions
            </div>
          </>
        )}
      </div>
