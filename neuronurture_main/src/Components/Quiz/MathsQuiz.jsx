...
const MathsQuiz = () => {
  ...
  const nextQuestion = () => {
    if (lock === true) {
      ...
      if (index === MathsQuizData.length - 1) {
        setResult(true);
        return 0; //so it not execute remaining statement
      }
      setIndex(++index);
      setQuestion(MathsQuizData[index]);
  ...
  const [items, setItems] = useState(() => {
    const savedItems = localStorage.getItem("NNMaths");
    return savedItems ? JSON.parse(savedItems) : [];
  });

  useEffect(() => {
    localStorage.setItem("NNMaths", JSON.stringify(items));
  }, [items]);
    ...
    <>
      ...
      <div className="container">
        ...
        {result ? (
          <>
            <h2>
              You scored {score} our of {MathsQuizData.length}
            </h2>
          ...
          <>
            ...
            <div className="index">
              {index + 1} of {MathsQuizData.length} questions
            </div>
          </>
        )}
      </div>
